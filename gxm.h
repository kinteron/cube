...
//velocity Factor in quaternionRotation
static float s_dampingFactor;
...
int initGxm(void)
{
	...
	s_basicVertices = (BasicVertex *)graphicsAlloc(SCE_KERNEL_MEMBLOCK_TYPE_USER_RWDATA_UNCACHE, 4 * 6 * 9 * sizeof(BasicVertex), 4, SCE_GXM_MEMORY_ATTRIB_READ, &s_basicVerticesUId);	//Eckpunkte
	s_basicIndices = (uint16_t *)graphicsAlloc(SCE_KERNEL_MEMBLOCK_TYPE_USER_RWDATA_UNCACHE, 6 * 6 * 9 * sizeof(uint16_t), 2, SCE_GXM_MEMORY_ATTRIB_READ, &s_basicIndiceUId);	//Dreiecke
...
void createGxmData(void)
{
	...
	//shader rotation ist 0 oder 1
	//my Rotation in shader
	const SceGxmProgramParameter *paramRotationAttribute = sceGxmProgramFindParameterByName(basicProgram, "aRotation");		//name in shader where aRotation is just a flag for each rotating point
	SCE_DBG_ALWAYS_ASSERT(paramRotationAttribute && (sceGxmProgramParameterGetCategory(paramRotationAttribute) == SCE_GXM_PARAMETER_CATEGORY_ATTRIBUTE));
	...
	basicVertexAttributes[2].format = SCE_GXM_ATTRIBUTE_FORMAT_U8; // Mapping relation clarified.
	basicVertexAttributes[2].componentCount = 1;	//value flag
	basicVertexAttributes[2].regIndex = sceGxmProgramParameterGetResourceIndex(paramRotationAttribute);
	...
	returnCode = sceGxmShaderPatcherCreateVertexProgram(s_shaderPatcher, s_basicVertexProgramId, basicVertexAttributes, 3, basicVertexStreams, 1, &s_basicVertexProgram);
	SCE_DBG_ALWAYS_ASSERT(returnCode == SCE_OK);
	...
	//zum VertexShader
	s_rotationParam = sceGxmProgramFindParameterByName(basicProgram, "rotate");
...
void renderGxm(void)
{
	...
	sceGxmSetUniformDataF(vertexDefaultBuffer, s_rotationParam, 0, 16, (float*)&s_rotationMatrix);


	/* draw the spinning triangle */
	sceGxmSetVertexStream(s_context, 0, s_basicVertices);
	sceGxmDraw(s_context, SCE_GXM_PRIMITIVE_TRIANGLES, SCE_GXM_INDEX_FORMAT_U16, s_basicIndices, 6 * 6 * 9);	// 9 squares
